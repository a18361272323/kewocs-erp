    ...
    <el-table v-loading="loading" :data="orderList" border stripe style="width: 100%">
      ...
      <el-table-column label="操作" width="280" fixed="right" align="center">
        <template #default="{ row }">
          ...
          <el-button v-if="row.orderStatus === 'CONFIRMED' || row.orderStatus === 'PARTIAL_OUT'" type="success" link @click="handleStockOut(row)">出库</el-button>
          <el-button v-if="row.orderStatus === 'OUT_STOCK' || row.orderStatus === 'PARTIAL_PAID'" type="warning" link @click="handleCollection(row)">收款</el-button>
          <el-button v-if="row.orderStatus === 'DRAFT'" type="info" link @click="handleEdit(row)">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
