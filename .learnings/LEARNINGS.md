...
## 2026-05-30: P0-P2 ??????????19??93????

**Category**: best_practice | **Priority**: critical | **Status**: resolved

### ????
25??????4????2?API???store?utils?docs ???????

### P0 ????
1. **StockIn????**?Promise.all ? Promise.allSettled?SN????????????+?????SN
2. **Check?????**?handleComplete/handleCompleteCheck ?? lostSns/profitSns ??????
3. **SaleSnSelector??**?catch??? ElMessage.error ????

### P1 ????
4. **??????**???? res.returnCode?res.code, res.code===0||200?res.code==='SUC0000', res.list?res.body?.list
5. **snStatus?status**?SnSelector/ScanStockIn/Inventory???????????????
6. **FINANCE_FORM_CODE**???????? '1154768285638656'???KNOWLEDGE_BASE.md?
7. **hasSn????**?product.hasSn===1 ? (product.hasSn===1||product.hasSn===true)

### P2 ????
8. **Dashboard??import**?await import('@/api') ? ??import
9. **.gitignore**??? tmp_*.py/tmp_*.txt ??????

### ????
- `request.js`??returnCode?code?????????????
- ?????????????????allSettled+????
- ????formCode=1154768285638656??????????????
- GBK/UTF-8?????Cloudflare??????????????Python rb/wb+UTF8.GetString
- ng?rg????????sn_code?snCode?convertParamsToSnakeCase?????
